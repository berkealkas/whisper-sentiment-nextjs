...
import { SPEAKERS, DEFAULT_SPEAKER } from '@utils/TTSConstants';
...
export default function TextToSpeech({ transcribedData }: Props) {
  ...
  useEffect(() => {
    if (!worker.current) {
      ...
      worker.current = new Worker(new URL('@workers/ttsWorker.js', import.meta.url), {
        type: 'module'
      });
    }
